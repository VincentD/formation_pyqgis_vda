...
from qgis.PyQt import QtCore, QtGui, uic, QtSql, Qt, QtWidgets
...
class bdhabnatDialog(QtWidgets.QDialog):
    ...
    def listesref(self):
        ...
        # Si le référentiel "Digitale" du CBNBL est sélectionné, alors remplir la combobox "cbx_hablat"
            ...
            #print (str(qcbnbl))
            if not ok:
                QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête CBNBL ratée')
                #print (qcbnbl)
            self.listefran()
        ...
        #Si un autre référentiel est sélectionné, alors remplir la combobox "cbx_habfr" avec les valeurs de la table correspondant au référentiel.
            ...
            qautrref = u"""select hab_cod, coalesce(hab_fr,' ') as hab_fr from bd_habnat.t_liste_ref_{zr_table} where hab_fr != '' order by hab_fr""".format(zr_table = str(self.habref))
            print (qautrref)
            ...
            if not ok:
                QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête Autre ref ratée')
    ...
    def listefran(self):
        ...
        if self.habref == 'cbnbl':
            ...
            #print ('listefran')
            if not ok :
                QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête Hab Fr ratée')
            while query_fr.next():
    ...
    def sauvSaisie(self):
        ...
        if entselect[0].type() == QgsWkbTypes.PolygonGeometry:
            typegeom='Polygon'
#            geom = self.transfoPoly(entselect)
#            print ("geom=" + str(geom.exportToWkt()))
        else: 
            QtWidgets.QMessageBox.warning(self, 'Alerte', u'Les entités sélectionnées ne sont pas des polygones')
            self.close
            return
        ...
        if coucheactive.crs().authid() == u'EPSG:2154':
            ...
            QtWidgets.QMessageBox.warning(self, 'Alerte', u'Les seules projections supportées sont le WGS84 et le Lambert93. Merci de vérifier la projection de votre couche SIG')
            return
        QgsProject.instance().addMapLayer(memlayer, False)
        ...
        for entite in geomlis :
            ...
            if memlayer.crs().authid() == u'EPSG:2154':
                thegeom='st_setsrid(st_geometryfromtext (\'{zr_geom}\'), 2154)'.format(zr_geom=entite.asWkt())
            elif memlayer.crs().authid() == u'EPSG:4326':
                thegeom='st_transform(st_setsrid(st_geometryfromtext (\'{zr_geom}\'),4326), 2154)'.format(zr_geom=entite.asWkt())
            else :
            ...
            if self.ui.chx_plantation.isChecked() == True :  #cas de la plantation
                ...
                if not ok:
                    QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête Mosaik ratée')
                if querymosaik.size() > 0 :
                    ...
                    sumprct += pourcent
                    if sumprct > 100 :
                        QtWidgets.QMessageBox.warning(self, 'Alerte', u'La somme des pourcentages des habitats dépasse 100 % dans la mosaïque')
                        self.iface.setActiveLayer(coucheactive)
                        QgsProject.instance().removeMapLayer(memlayer.id())
                        return
                    querymosaik.first()
                    ...
                    if pourcent > 99 :
                        ...
                        if not ok:
                            QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête PlusGrandIdMosaik ratée')
                        querybiggestid.next()
            ...
            if self.habref == 'cbnbl':
                ...
                if not okrarmen :
                    QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête RarMen ratée')
                queryrarmen.next()
                ...
                if not okpatcen :
                    QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête PatCen ratée')
                querypatcen.next()
            ...
            if not ok:
                QtWidgets.QMessageBox.warning(self, 'Alerte', u'Requête sauver Ope ratée')
        ...
        if self.erreurSaisieBase == '0':
            QtWidgets.QMessageBox.information(self, 'Information', u'Données correctement saisies dans la base')
        else : 
            QtWidgets.QMessageBox.warning(self, 'Alerte', u'Il y a eu une erreur lors de la saisie. Données non saisies en base.')
        self.close()
